...
void ili9341_rounder(lv_disp_drv_t * disp_drv, lv_area_t *area)
{
    // area->x1 = 0;
    // area->x2 = 200 - 1;
...
void ili9341_init(void)
{
	...
	// uint8_t command = 0x36; // Command for Memory Access Control
    // uint8_t data = 0x48; // Base value for MAC register

    // if (horizontal) {
    //     data |= (1 << 6); // Set MX bit for horizontal mirroring
    // }

    // if (vertical) {
    //     data |= (1 << 5); // Set MV bit for vertical mirroring
    // }

    // // Send the command and data to the ILI9341
    // spi_send_command(command);
	...
	uint8_t my_command = 0x36; // Command for Memory Access Control
    uint8_t my_data = 0x48; // Base value for MAC register

	my_data |= (1 << 7);
	// my_data |= (1 << 5);

	ili9341_send_cmd(my_command);
	ili9341_send_data(&my_data, 1);
}
...
void ili9341_flush(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_map)
{
	...
	// ESP_LOGW(TAG, "Flush - sending data");
	// ESP_LOGW(TAG, "Width: %d, Height: %d", lv_area_get_width(area), lv_area_get_height(area));

	// ili9341_send_color((void*)color_map, 320 * 2 * lv_area_get_height(area));

	for(size_t row = 0; row <= ((lv_area_get_height(area) / 16) - 1); row++) {
		// ssd1680_send_data(buffer, linelen);
		ili9341_send_color((void*)color_map, 320 * 2 * 16);
		color_map += 320 * 16; //(128/8)x296 = 4736
		// vTaskDelay(pdMS_TO_TICKS(1));
	}
	
	// ili9341_send_color((void*)color_map + (size * 8), size * 8);
