...
{
    // ESP_LOGW(TAG, "");
    // ESP_LOGW(TAG, "\n\nFlush area: x1: %d, x2: %d, y1: %d, y2: %d", area->x1, area->x2, area->y1, area->y2);
    ...
    if (!partial_counter) {
        ssd1680_init();
        // ESP_LOGW(TAG, "Refreshing in FULL");

        ssd1680_set_window(0, EPD_PANEL_WIDTH - 1, 0, EPD_PANEL_HEIGHT - 1);
        // ssd1680_set_cursor(area->y1,  EPD_PANEL_HEIGHT - area->x1 - 1);
        ...
        // ESP_LOGW(TAG, "Refreshing in PARTIAL");
        //update partial
        ssd1680_hw_reset();
        ssd1680_write_cmd(SSD1680_CMD_BWF_CTRL, ssd1680_border_init, 1);
        // ESP_LOGW(TAG, "area->x1: %d, area->x2: %d, area->y1: %d, area->y2: %d", area->x1, area->x2, area->y1, area->y2);
        
        ssd1680_set_window(0, EPD_PANEL_WIDTH - 1, 0, EPD_PANEL_HEIGHT - 1);
        ssd1680_set_cursor(area->y1, area->x1);
        // ssd1680_set_cursor(0,  0);


        ssd1680_send_cmd(SSD1680_CMD_WRITE1_RAM);
        // uint8_t buffer2 = 0b000000;
        for(size_t row = 0; row <= (EPD_PANEL_HEIGHT - 1); row++) {
            ssd1680_send_data(buffer, linelen);
            buffer += SSD1680_COLUMNS; //(128/8)x296 = 4736
        }
        // for(size_t row = 0; row <= 10; row++) {
        //     ssd1680_send_data(&buffer2, 1);
        //     buffer2++;
        //     buffer += SSD1680_COLUMNS; //(128/8)x296 = 4736
        // }
...
static inline void ssd1680_set_window( uint16_t sx, uint16_t ex, uint16_t ys, uint16_t ye)
{
    ...
    // ESP_LOGW(TAG, "sx: %d, ex: %d, ys: %d, ye: %d", sx, ex, ys, ye);
    uint8_t tmp[4] = {0};
    ...
    // ESP_LOGW(TAG, "tmp0: %d, tmp1: %d", tmp[0], tmp[1]); 
    ...
    // ESP_LOGW(TAG, "tmp0: %d, tmp1: %d, tmp2: %d, tmp3: %d", tmp[0], tmp[1], tmp[2], tmp[3]); 
    ...
    lv_color_t color, lv_opa_t opa)
{
    ...
    uint16_t mirrored_idx = (x+1) + (4440 - ((y >> 3) * EPD_PANEL_HEIGHT));
    bit_index  = y & 0x7;
    /* 1 means white, 0 means black */
    /* note that the bit index is inverted in place */
    // ! filip: changed 0 to 1
    if (color.full == 1) {
        BIT_SET(buf[mirrored_idx - 1], bit_index);
    } else {
        BIT_CLEAR(buf[mirrored_idx - 1], bit_index);
    }
#else
...
void ssd1680_init(void)
{
    ...
    esp_rom_gpio_pad_select_gpio(SSD1680_RST_PIN);
